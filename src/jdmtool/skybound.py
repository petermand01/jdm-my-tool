...
class SkyboundDevice():
    ...
    def read_database(self, fd: BinaryIO, progress_cb: Callable[[int], None]) -> None:
        self.before_read()
        for i in range(self.get_total_pages() * SkyboundDevice.BLOCKS_PER_PAGE):
            self._loop_helper(i)
    ...
    def erase_database(self, pages: int, progress_cb: Callable[[int], None]) -> None:
        ...
        self.select_page(self.get_total_pages() - 1)
        self.erase_page()
